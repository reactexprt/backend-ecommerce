...
router.get('/', authenticateToken, async (req, res) => {
  try {
    ...
    let cart = await Cart.findOne({ user: req.user.userId })
      .populate('items.productId', 'name price images discountPrice stock') // Populate only necessary fields
      .exec();
...
router.post('/', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const updatedCart = await Cart.findOne({ user: userId }).populate('items.productId', 'name price images discountPrice stock');
...
router.delete('/:productId', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const updatedCart = await Cart.findOne({ user: userId }).populate('items.productId', 'name price images discountPrice stock').session(session);
